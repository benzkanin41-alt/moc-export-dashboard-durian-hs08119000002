...
def extract_hs_versions(rows: Any) -> list[str]:
    versions: list[str] = []
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue
        for key in ("id", "text", "name", "Name", "value"):
            value = row.get(key)
...
def extract_hs_name(lookup: Any) -> str:
    candidates: list[dict[str, Any]] = []
    if isinstance(lookup, dict):
        candidates.append(lookup)
        for key in ("data", "results", "items"):
            nested = lookup.get(key)
            if isinstance(nested, list):
                candidates.extend(item for item in nested if isinstance(item, dict))
    elif isinstance(lookup, list):
        candidates.extend(item for item in lookup if isinstance(item, dict))

    exact_candidates = []
    for item in candidates:
        item_id = str(item.get("id") or item.get("ID") or item.get("code") or item.get("Code") or "")
        if item_id == HS_CODE:
            exact_candidates.append(item)
    search_order = exact_candidates or candidates
    for item in search_order:
        for key in ("Name", "name", "text", "Text", "description", "Description"):
            value = item.get(key)
            if value is not None and str(value).strip():
...
def find_hscode(session: requests.Session) -> tuple[str, str, Any]:
    version_rows = request_json(session, "GET", f"{BASE_URL}/lookup/hscodeversions")
    versions = [PREFERRED_HS_VERSION]
    for version in extract_hs_versions(version_rows):
        if version not in versions:
            versions.append(version)

    lookup_errors: list[str] = []
    for version in versions:
        url = f"{BASE_URL}/lookup/hscodelist{HS_CODE}/{version}"
        try:
            lookup = request_json(session, "POST", url, json_payload={"lang": "th"})
        except Exception as exc:  # noqa: BLE001 - keep trying published versions.
            lookup_errors.append(f"{version}: {exc}")
            continue
        hs_name = extract_hs_name(lookup)
        if hs_name:
            return version, hs_name, lookup
        lookup_errors.append(f"{version}: empty lookup response")
    raise RuntimeError(f"HS Code {HS_CODE} not found in MOC lookup. Tried: {', '.join(lookup_errors)}")


def make_payload(year: int, month: int, year_name: Any, hs_name: str, hs_version: str) -> dict[str, Any]:
    return {
...
def main() -> None:
    ...
    hs_version, hs_name, hs_lookup = find_hscode(session)
